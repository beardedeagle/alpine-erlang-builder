.PHONY: help version test erl shell build clean rebuild release
.NOTPARALLEL: build rebuild release
.DEFAULT_GOAL := help

VERSION ?= `cat VERSION`
MIN_VERSION := $(shell echo $(VERSION) | sed 's/\([0-9][0-9]*\)\.\([0-9][0-9]*\)\(\.[0-9][0-9]*\)*/\1.\2/')
IMAGE_NAME ?= beardedeagle/alpine-erlang-builder
...
version: ## Show Makefile version
	@echo -e 'OTP/Erlang version: $(shell echo $(VERSION))'
...
erl: ## Run erl shell in Docker image
